...
class ThermalPrinter {
  constructor(initConfig) {
    ...
    this.disableExternalBuzzer();
  }

  disableExternalBuzzer() {
    this.printer.append(this.printer.config.DISABLE_BUZZER);
  }
  ...
  leftRight(left, right) {
    ...
    var width = this.config.width - this._calcWidth(left) - this._calcWidth(right);
  ...
  table(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var spaces = cellWidth - this._calcWidth(data[i]);
      console.log(spaces)
  ...
  tableCustom(data) {
    var cellWidth = this.config.width / data;
    var secondLine = [];
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (cellWidth < this._calcWidth(obj.text)) {
        tooLong = true;
      ...
      if (obj.align == "CENTER") {
        var spaces = (cellWidth - this._calcWidth(obj.text)) / 2;
        for (var j = 0; j < spaces; j++) {
          this.append(Buffer.from(" "));
        }
        ...
        var spaces = cellWidth - this._calcWidth(obj.text);
        for (var j = 0; j < spaces; j++) {
          this.append(Buffer.from(" "));
        }
        if (obj.text != '') this.append(obj.text);
...
module.exports = { ThermalPrinter, PrinterTypes } ;
